...
def send_message(client: socket.socket, disconnect: threading.Event) -> None:
    while True:
        try:
            message = input().strip()
...
def recieve_message(client: socket.socket, disconnect: threading.Event) -> None:
    while True:
        try:
...
def disconnect_client(client: socket.socket, disconnect: threading.Event) -> None:
    disconnect.set()
    client.close()
    print("\n-= See you later! =-")
...
def start_send_recieve_threads(client: socket.socket) -> None:
    ...
    t1 = threading.Thread(target=send_message, args=(client, disconnect))
    t1.start()

    t2 = threading.Thread(target=recieve_message, args=(client, disconnect))
    t2.daemon = True
    t2.start()
